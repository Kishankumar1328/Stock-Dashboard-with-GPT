...
if not ticker:
    ...
    if data.empty:
        ...
        st.subheader(f"Bar Chart - {ticker}")
        fig_bar = px.bar(data, x=data.index, y='Volume', title=f"{ticker} Volume")
        st.plotly_chart(fig_bar, use_container_width=True)

        # Display Line Chart
        st.subheader(f"Line Chart - {ticker}")
        fig_line = px.line(data, x=data.index, y='Adj Close', title=f"{ticker} Adj Close Price")
        st.plotly_chart(fig_line, use_container_width=True)

        # Display Point and Figure Chart
        st.subheader(f"Point and Figure Chart - {ticker}")
        # A simple representation using a scatter plot
        fig_pnf = px.scatter(data, x=data.index, y='Adj Close', title=f"{ticker} Point and Figure Chart")
        st.plotly_chart(fig_pnf, use_container_width=True)

        # Display Pie Chart
        st.subheader(f"Pie Chart - Trading Volume Distribution")
        trading_volume_by_day = data['Volume'].groupby(data.index.day_name()).sum()
        fig_pie = px.pie(names=trading_volume_by_day.index, values=trading_volume_by_day.values,
                         title=f"Trading Volume Distribution by Day")
